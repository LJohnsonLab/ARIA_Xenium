...
# DO NOT RUN: Code to save the marker genes list for future reference
# Marker genes for various microglia states based on the previous single cell experiments with ARIA samples. 
# Filtered to include only those present in the current panel.
marker_genes <- list(
...
```{r}
# Marker genes used for module score calculation
mk <- read_csv("data/JRZFVH_mBrain_480g_gene_list_with_microglial_states_JM.csv")
marker_genes <- mk |>
  filter(!is.na(microglia_state)) |>
  group_by(microglia_state) |>
